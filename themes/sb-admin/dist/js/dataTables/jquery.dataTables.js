...
/**
 * @summary     DataTables
 * @description Paginate, pencarian and order HTML tables
 * @version     1.10.0-dev
...
(/** @lends <global> */function( $ ) {
	...
	function _fnFeatureHtmlFilter ( settings )
	{
		...
		var input = '<input type="pencarian" class="'+classes.sFilterInput+'"/>';
		...
		var jqFilter = $('input[type="pencarian"]', filter)
			.val( previousSearch.sSearch.replace('"','&quot;') )
			.bind( 'keyup.DT pencarian.DT input.DT paste.DT cut.DT', function(e) {
	 ...
	 *  @param {object} oSettings dataTables settings object
	 *  @param {object} oSearch pencarian information
	...
	function _fnFilterComplete ( oSettings, oInput, iForce )
	{
		...
		_fnCallbackFire( oSettings, null, 'pencarian', [oSettings] );
	}
	...
	{
		var afnFilters = DataTable.ext.pencarian;
	 ...
	 *  @param {bool} bRegex treat pencarian string as a regular expression or not
	...
	function _fnFilter( settings, input, force, regex, smart, caseInsensitive )
	{
		...
		if ( DataTable.ext.pencarian.length !== 0 ) {
			force = true;
		}
		...
		if ( input.length <= 0 ) {
			...
			// New pencarian - start from the master array
			if ( invalidated ||
				 force ||
	 ...
	 *  @param {string} sSearch string to pencarian for
	...
	function _fnFilterData ( settings )
	{
		...
		var fomatters = DataTable.ext.type.pencarian;
	...
	DataTable = function( options )
	{
		 ...
		 *      oTable.$('tr', {"pencarian": "applied"}).css('backgroundColor', 'blue');
		 *      oTable.fnFilter('');
		 *    } );
		 */
		 ...
		 *      oTable.fnFilter('Webkit');
		 *      var data = oTable._('tr', {"pencarian": "applied"});
		 *
		 *      // Do something with the data
		 *      alert( data.length+" rows matched the pencarian" );
		 *    } );
		 */
		...
		this.fnFilter = function( sInput, iColumn, bRegex, bSmart, bShowGlobal, bCaseInsensitive )
		{
			...
			if ( iColumn === null || iColumn === undefined ) {
				api.pencarian( sInput, bRegex, bSmart, bCaseInsensitive );
			}
			else {
				api.column( iColumn ).pencarian( sInput, bRegex, bSmart, bCaseInsensitive );
			}
			...
			if ( rowOne.length ) {
				...
				$.each( _fnGetRowElements( oSettings, rowOne[0] ).cells, function (i, cell) {
					...
					if ( col.mData === i ) {
						...
						var filter = a( cell, 'filter' ) || a( cell, 'pencarian' );
	...
	var _selector_opts = function ( opts )
	{
		...
		// than pencarian
		if ( opts.filter && ! opts.pencarian ) {
			opts.pencarian = opts.filter;
		}
	
		return {
			pencarian: opts.pencarian || 'none',
	...
	var _selector_row_indexes = function ( settings, opts )
	{
		...
		var
			pencarian = opts.pencarian,  // none, applied, removed
			order  = opts.order,   // applied, current, index (original - compatibility with 1.9)
			page   = opts.page;    // all, current
	
		// Current page implies that order=current and fitler=applied, since it is
		// fairly senseless otherwise, regardless of what order and pencarian actually
		// are
		if ( page == 'current' )
		{
			...
			a = pencarian == 'none' ?
				displayMaster.slice() :                      // no pencarian
				pencarian == 'applied' ?
					displayFiltered.slice() :                // applied pencarian
					$.map( displayMaster, function (el, i) { // removed pencarian
			...
			for ( i=0, ien=settings.aoData.length ; i<ien ; i++ ) {
				if ( pencarian == 'none' ) {
					...
					if ((tmp === -1 && pencarian == 'removed') ||
						(tmp === 1  && pencarian == 'applied') )
					{
						a.push( i );
					}
				}
	...
	_api_registerPlural( 'rows().cache()', 'row().cache()', function ( type ) {
		return this.iterator( 'row', function ( settings, row ) {
			...
			return type === 'pencarian' ? r._aFilterData : r._aSortData;
		} );
	} );
	...
	_api_registerPlural( 'rows().remove()', 'row().remove()', function () {
		...
		return this.iterator( 'row', function ( settings, row, thatIdx ) {
			...
			// Remove the target row from the pencarian array
	...
	_api_registerPlural( 'columns().cache()', 'column().cache()', function ( type ) {
		return this.iterator( 'column-rows', function ( settings, column, i, j, rows ) {
			return _pluck_order( settings.aoData, rows,
				type === 'pencarian' ? '_aFilterData' : '_aSortData', column
			);
		} );
	} );
	...
	_api_registerPlural( 'cells().cache()', 'cell().cache()', function ( type ) {
		type = type === 'pencarian' ? '_aFilterData' : '_aSortData';
	...
	_api_register( 'pencarian()', function ( input, regex, smart, caseInsen ) {
		var ctx = this.context;
	 ...
	 * pencarian information for the global filter and individual column filters.
	 *  @namespace
	 */
	DataTable.models.oSearch = {
		...
		/**
		 * Flag to indicate if the pencarian term should be interpreted as a
	...
	DataTable.models.oRow = {
		 ...
		 * needed on every pencarian (memory traded for performance)
		 *  @type array
		 *  @default null
		 *  @private
		 */
	...
	DataTable.defaults = {
		...
		/**
		 * Basically the same as `pencarian`, this parameter defines the individual column
		 * filtering state at initialisation time. The array must be of the same size
		 * as the number of columns, and each element be an object with the parameters
		 * `pencarian` and `escapeRegex` (the latter is optional). 'null' is also
		 ...
		 *          { "pencarian": "My filter" },
		 *          null,
		 *          { "pencarian": "^[0-9]", "escapeRegex": false }
		 *        ]
		 *      } );
		 *    } )
		 */
		 ...
		 * the page number, so if you have 10 data per halaman and want to start on
		...
		"oLanguage": {
			 ...
			 *    } );
			 */
			"sEmptyTable": "Tidak ada kecocokan data pada tabel",
			 ...
			 *
			 *  @dtopt Language
			 *  @name DataTable.defaults.language.pencarian
			 *
			 *  @example
			 ...
			 *      $('#example').dataTable( {
			 *        "language": {
			 *          "pencarian": "Apply filter _INPUT_ to table"
			 *        }
			 *      } );
			 *    } );
			 */
		 ...
		 * initialisation time. As an object the `pencarian` parameter must be
		 * defined, but all other parameters are optional. When `regex` is true,
		 * the pencarian string will be treated as a regular expression, when false
		 ...
		 *
		 *  @dtopt Options
		 *  @name DataTable.defaults.pencarian
		 *
		 *  @example
		 *    $(document).ready( function() {
		 *      $('#example').dataTable( {
		 *        "pencarian": {"pencarian": "Initial pencarian"}
		 *      } );
		 *    } )
		 */
	...
	DataTable.models.oSettings = {
		...
		/**
		 * Store the applied global pencarian information in case we want to force a
		 * research or compare the old pencarian to a new one.
		 * Note that this parameter will be set by the initialisation routine. To
		 * set a default use {@link DataTable.defaults}.
		 *  @namespace
		 *  @extends DataTable.models.oSearch
		...
		/**
		 * Store the applied pencarian for each column - see
	...
	DataTable.ext = _ext = {
		 ...
		 *   (false)
		 *
		 * Note that as with the main pencarian ability in DataTables, technically this
		 ...
		 *
		 *  @example
		 *    // The following example shows custom pencarian being applied to the
		 *    // fourth column (i.e. the data[3] index) based on two input values
		 *    // from the end-user, matching the data in a certain range.
		 *    $.fn.dataTable.ext.pencarian.push(
		 ...
		 * The type of a column will effect how it is ordering and pencarian (plug-ins
		...
		type: {
			...
			/**
			 * Type based pencarian formatting.
			 *
			 * The type based searching functions can be used to pre-format the
			 * data to be pencarian on. For example, it can be used to strip HTML
			 * tags or to de-format telephone numbers for numeric only searching.
			 *
			 * Note that is a pencarian is not defined for a column of a given type,
			 * no pencarian formatting will be performed.
			 * 
			 ...
			 *
			 *  @example
			 *    $.fn.dataTable.ext.type.pencarian['title-numeric'] = function ( d ) {
			 *      return d.replace(/\n/g," ").replace( /<.*?>/g, "" );
			 *    }
			 */
			pencarian: {},
	...
	$.extend( DataTable.ext.type.pencarian, {
		html: function ( data ) {
			return _empty(data) ?
				'' :
				typeof data === 'string' ?
	...
	/**
	 * pencarian event, fired when the searching applied to the table (using the
	 * built-in global pencarian, or column filters) is altered.
	 *  @name DataTable#pencarian.dt
	 *  @event
	 *  @param {event} e jQuery event object
	 *  @param {object} o DataTables settings object {@link DataTable.models.oSettings}
	 */
